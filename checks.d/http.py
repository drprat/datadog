...
class HTTPCheck(AgentCheck):
    def check(self, instance):
        ...
        if r.status_code != 200:
            self.status_code_event(url, r,timeout, aggregation_key)
    ...
    def status_code_event(self, url, r,timeout, aggregation_key):
        self.event({
            'timestamp': int(time.time()),
            'event_type': 'http_check',
            'msg_title': 'Invalid reponse code for %s' % url,
            'msg_text': '%s returned a status of %s after %s seconds' % (url, r.status_code,timeout),
            'aggregation_key': aggregation_key
        })
